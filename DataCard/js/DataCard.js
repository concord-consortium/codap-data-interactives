...
var dataManager = Object.create({
  ...
  requestCaseByIndex: function (contextName, collectionName, collectionIndex) {
    dispatcher.sendRequest({
      action: 'get',
      resource: 'dataContext[' + contextName + '].collection[' + collectionName + '].caseByIndex[' + collectionIndex + ']'
    });
  },
  ...
  requestCaseByID: function (dataContext, collectionName, caseID, select) {
    dispatcher.sendRequest({
      action: 'get',
      resource: 'dataContext[' + dataContext + '].collection[' + collectionName + '].caseByID[' + caseID + ']'
  ...
  setCase: function (iCollectionName, values, resourceName, handlingOptions) {
    ...
    if (collectionIndex >= 0) {
      ...
      if (collectionIndex > 0) {
        guaranteeLeftCollectionIsParent(collectionInfoList[collectionIndex - 1], myCase.parent, this.state.currentContext);
      }
  ...
  setDirty: function (collectionInfo, isDirty) {
    ...
    this.state.isDirty = this.state.collectionInfoList.reduce(function (sum, collectionInfo) {
  ...
  didUpdateCase: function (iCollectionName, iReply) {
    ...
    if (collectionInfo) {
      if (collectionInfo.currentCaseIsNew) {
        ...
        this.requestCaseByID(this.state.currentContext, collectionInfo.collection.name, iReply.values[0].id, true);
      }
      collectionInfo.currentCaseIsNew = false;
      this.setDirty(collectionInfo, false);
      this.notify();
  ...
  startNewCase: function (iCollectionName) {
    ...
    function makeNewCase(collectionInfo, parentCollectionInfo, contextName) {
      ...
      collectionInfo.currentCaseResourceName = 'dataContext[' + contextName + '].collection[' + collectionInfo.collection.name + '].case';
  ...
  cancelCreateOrUpdateCase: function () {
    this.state.collectionInfoList.forEach(function (collectionInfo) {
      if (collectionInfo.isDirty) {
        if (collectionInfo.currentCaseIsNew) {
          this.requestCaseByIndex(this.state.currentContext, collectionInfo.collection.name, collectionInfo.currentCaseIndex);
          this.setDirty(collectionInfo, false);
        } else {
          dispatcher.sendRequest({
            action: 'get',
  ...
  handleResponse: function (request, result, handlingOptions) {
    ...
    if (!result) {
      ...
      switch (resourceObj.type) {
        ...
        case 'caseByIndex':
        case 'caseByID':
          dataManager.setCase(resourceObj.collection, result.values, request.resource, handlingOptions);
          break;
        case 'selectionList':
  ...
  render: function () {
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement("input", { type: "button", className: "update-case",
  ...
  render: function () {
    ...
    return React.createElement(
      ...
      React.createElement(
        "div",
        { className: "left-ctls" },
        React.createElement(CaseNavControl, { action: "prev", navEnabled: navEnabled.prev,
      ...
      React.createElement(
        ...
        React.createElement(CaseNavControl, { action: "next", navEnabled: navEnabled.next,
          onNavigation: this.navigationHandler }),
        React.createElement(CaseNavControl, { action: "new", navEnabled: navEnabled.newInstance,
          onNavigation: this.navigationHandler }),
        React.createElement(CaseNavControl, { action: "remove", navEnabled: navEnabled.removeInstance,
          onNavigation: this.navigationHandler })
      )
    );
  }
